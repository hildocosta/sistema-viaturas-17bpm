...
export default function NovaViaturaPage() {
  ...
  const inputClass = "w-full px-3 py-1.5 bg-slate-950/70 border border-slate-700/80 rounded-lg text-slate-100 placeholder-slate-500 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all hover:border-slate-600";
  const labelClass = "block text-[11px] font-semibold uppercase tracking-wider text-slate-300 mb-1";

  return (
    <DashboardWrapper className="h-screen overflow-hidden">
      <SidebarArea>
        <Sidebar />
      </SidebarArea>

      <MainContent className="h-full flex flex-col justify-between overflow-hidden p-6">
        <PageHeader
          icon={Car}
        ...
        <div className="flex-1 flex flex-col justify-center max-w-4xl mx-auto w-full space-y-4 my-auto">
          <div className="bg-slate-900/60 px-4 py-2.5 rounded-xl border border-slate-800 shadow-sm backdrop-blur-sm">
