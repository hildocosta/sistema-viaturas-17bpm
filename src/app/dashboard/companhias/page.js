...
export default function DistribucionCompanhiasPage() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "Operacional":
        return <span className="px-2 py-0.5 text-[10px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full flex items-center gap-1 w-fit"><CheckCircle2 size={12}/> Operacional</span>;
      case "Manutenção":
        return <span className="px-2 py-0.5 text-[10px] font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-full flex items-center gap-1 w-fit"><Wrench size={12}/> Manutenção</span>;
      case "Baixada":
        return <span className="px-2 py-0.5 text-[10px] font-semibold bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-full flex items-center gap-1 w-fit"><XCircle size={12}/> Baixada</span>;
      default:
        return null;
    }
  };
      ...
      <main className="flex-1 h-full bg-slate-900 rounded-xl sm:rounded-2xl border border-slate-800 p-3 sm:p-6 flex flex-col overflow-y-auto relative">
        <div className="max-w-7xl mx-auto w-full space-y-4 sm:space-y-5">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 pb-3 sm:pb-4 border-b border-slate-800">
            <div>
              <h1 className="text-lg sm:text-xl font-bold text-slate-100 flex items-center gap-2">
                <Building2 className="text-blue-500 shrink-0" size={22} />
                <span className="truncate">Distribuição por Companhias</span>
              </h1>
              <p className="text-xs text-slate-400 mt-0.5">17º Batalhão de Polícia Militar — Visão da frota</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-xl gap-2 text-xs p-4 text-center">
              <AlertCircle size={24} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {companhiasFiltradas.length === 0 ? (
                <div className="col-span-full flex flex-col items-center justify-center py-12 text-slate-400 text-xs bg-slate-950/40 rounded-xl border border-slate-800/60 p-4 sm:p-6 gap-3 text-center">
                  <p className="font-semibold text-slate-300">Nenhum resultado para o filtro selecionado.</p>
                  
                  <div className="w-full max-w-lg bg-slate-950 p-3 sm:p-4 rounded-lg border border-slate-800 text-left overflow-x-auto text-[11px] font-mono text-slate-400">
                ...
                companhiasFiltradas.map((cia, index) => {
                  ...
                  return (
                    <div
                      ...
                      className="bg-slate-950/60 border border-slate-800 hover:border-blue-500/50 rounded-xl p-3.5 sm:p-4 transition-all hover:shadow-lg hover:shadow-blue-600/5 cursor-pointer group flex flex-col justify-between"
                    >
                      <div>
                        <div className="flex items-start justify-between mb-3 gap-2">
                          <div className="min-w-0 flex-1">
                            <span className="text-[10px] font-bold text-blue-400 uppercase tracking-wider bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20 inline-block mb-1">
                              {cia.sigla || "PM"}
                            </span>
                            <h2 className="text-sm font-bold text-slate-100 group-hover:text-blue-400 transition-colors truncate">
                            ...
                            <p className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5 truncate">
                              ...
                              <span className="truncate">{cia.areaAtuacao || cia.cidade || "Área 17º BPM"}</span>
                            </p>
                          </div>
                        ...
                        {/* Grid de Estatísticas das Viaturas */}
                        <div className="grid grid-cols-4 gap-1 my-3 bg-slate-900/80 border border-slate-800/80 rounded-xl p-2">
                          <div className="text-center border-r border-slate-800/80 pr-0.5">
                            <p className="text-[8px] sm:text-[9px] text-slate-500 uppercase font-semibold">Total</p>
                            <p className="text-sm sm:text-base font-bold text-slate-100">{total}</p>
                          </div>
                          <div className="text-center border-r border-slate-800/80 pr-0.5">
                            <p className="text-[8px] sm:text-[9px] text-emerald-400 uppercase font-semibold">Prontas</p>
                            <p className="text-sm sm:text-base font-bold text-emerald-400">{operacionais}</p>
                          </div>
                          <div className="text-center border-r border-slate-800/80 pr-0.5">
                            <p className="text-[8px] sm:text-[9px] text-amber-400 uppercase font-semibold">Manten.</p>
                            <p className="text-sm sm:text-base font-bold text-amber-400">{manutencao}</p>
                          </div>
                          <div className="text-center">
                            <p className="text-[8px] sm:text-[9px] text-rose-400 uppercase font-semibold">Baixas</p>
                            <p className="text-sm sm:text-base font-bold text-rose-400">{baixadas}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-800/60 text-xs text-slate-400 gap-2">
                        <span className="flex items-center gap-1 text-[10px] text-slate-500 truncate">
                          ...
                          <span className="truncate">{cia.comandante || "Comando do Batalhão"}</span>
                        </span>
        ...
        {companhiaSelecionada && (
          <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
            <div className="w-full lg:max-w-xl bg-slate-900 border-l border-slate-800 h-full p-4 sm:p-6 flex flex-col justify-between shadow-2xl overflow-y-auto">
              
              <div>
                <div className="flex items-start justify-between pb-4 border-b border-slate-800 mb-4 gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="p-2 sm:p-2.5 bg-blue-600/10 border border-blue-500/20 rounded-xl text-blue-400 shrink-0">
                      <Building2 size={20} />
                    </div>
                    <div className="min-w-0">
                      <h2 className="text-sm sm:text-base font-bold text-slate-100 truncate">{companhiaSelecionada.nome}</h2>
                      <p className="text-[11px] sm:text-xs text-slate-400 truncate">
                        {companhiaSelecionada.areaAtuacao || companhiaSelecionada.cidade || "Área 17º BPM"}
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-2.5">
                  ...
                  {companhiaSelecionada.viaturas
                    ...
                    .map((v) => (
                      <div 
                        key={v.id || v.prefixo}
                        className="bg-slate-950/80 border border-slate-800/80 rounded-xl p-3 flex items-center justify-between gap-2 hover:border-slate-700 transition-all"
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-xs sm:text-sm font-bold text-slate-100">{v.prefixo}</span>
                              <span className="text-[9px] font-mono text-slate-400 bg-slate-900 px-1 py-0.2 rounded border border-slate-800">{v.placa}</span>
                            </div>
                            <p className="text-[11px] text-slate-400 truncate mt-0.5">{v.modelo} • <span className="text-slate-500">{v.tipo || "Geral"}</span></p>
                          </div>
                        </div>
